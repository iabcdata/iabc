...
if [ ! -z "$PID_EXIST" ];then
	techo "Stopping the democ-service . pid:${PID}";
...
#ps axfww | grep democ-service | grep -v grep | awk '{print $1}' | xargs kill -9
#ps axww | grep democ-service | grep -v grep | awk '{print $1}' | xargs kill -9
